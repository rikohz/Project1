 ...
 * @property Userlistcontent[] $userlistcontents
 */
class Userlist extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('idUser, name, createDate', 'required'),
			array('public', 'numerical', 'integerOnly'=>true),
			array('idUser', 'length', 'max'=>20),
			array('name', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('idUserList, idUser, name, public, createDate', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'userListContents' => array(self::HAS_MANY, 'Userlistcontent', 'idUserList'),
		);
	}
